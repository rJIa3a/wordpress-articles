...
def run(folder,before_name='refinement-morphology',after_name='refinement-sentences'):
    ...
    result=dict(before=before_name,after=after_name,split='dev_only',resamples=2000,unit='source_article',caveat='Descriptive only: models selected on this dev set; articles may be topically dependent. Not a new holdout.',delta={name:dict(point=float(point[i]),interval95=np.quantile(deltas[:,i],[.025,.975]).tolist()) for i,name in enumerate(['precision','recall','f05'])})
    (d/after_name/'uncertainty.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--data',default='local-data/cities');p.add_argument('--before',choices=['refinement-morphology','refinement-sentences'],default='refinement-morphology');p.add_argument('--after',choices=['refinement-sentences','refinement-ranking'],default='refinement-sentences');a=p.parse_args();run(a.data,a.before,a.after)
