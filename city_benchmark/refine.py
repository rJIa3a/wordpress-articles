...
def run(folder):
    ...
    configs=[('baseline_logistic',LogisticRegression(C=.5,max_iter=1000,random_state=17),8),
             ('context_logistic',LogisticRegression(C=.5,max_iter=1000,random_state=17),X.shape[1]),
             ('context_trees',HistGradientBoostingClassifier(max_iter=150,max_leaf_nodes=7,min_samples_leaf=60,l2_regularization=20,learning_rate=.06,early_stopping=False,random_state=17),X.shape[1]-2),
             ('entity_context_trees',HistGradientBoostingClassifier(max_iter=150,max_leaf_nodes=7,min_samples_leaf=60,l2_regularization=20,learning_rate=.06,early_stopping=False,random_state=17),X.shape[1])]
    for name,model,n in configs:
        model.fit(X[train,:n],y[train]);scores=model.predict_proba(X[dev,:n])[:,1]
